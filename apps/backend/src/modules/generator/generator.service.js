...
const mapStudentVariables = (student, globalData = {}) => {
  ...
  // Filter out empty/blank values from user input so they don't override existing data
  const filteredGlobalData = {};
  for (const [key, value] of Object.entries(globalData)) {
    if (Array.isArray(value)) {
      // Keep arrays (like ranking tables) only if they have at least one non-empty row
      const nonEmptyRows = value.filter(row => 
        typeof row === 'object' && row !== null && 
        Object.values(row).some(v => v !== undefined && v !== null && String(v).trim() !== '')
      );
      if (nonEmptyRows.length > 0) {
        filteredGlobalData[key] = nonEmptyRows;
      }
    } else if (value !== undefined && value !== null && String(value).trim() !== '') {
      filteredGlobalData[key] = value;
    }
  }

  // Merge: user input (filteredGlobalData) takes highest priority,
  // then custom extraData, then standard fields, then raw student fields
  const mergedData = {
    ...
    ...filteredGlobalData, // user input from form (highest priority, empty values excluded)
  };
...
/**
 * Remove empty paragraphs at the end of the HTML to prevent large gaps before signature
 */
const stripTrailingEmptyParagraphs = (html) => {
  if (!html) return html;
  const regex = /(?:<p[^>]*>\s*(?:<br\s*\/?>|&nbsp;|\s)*\s*<\/p>\s*)+$/i;
...
handlebars.registerHelper('index_1', (options) => {
  return options.data.index + 1;
});

/**
 * Fix Handlebars {{#each}} blocks inside tables generated by Tiptap.
 * Tiptap wraps each element in its own <td>. This moves the {{#each}} to wrap the <tr> 
 * and removes empty <tr> that Tiptap might generate.
 * @param {string} html 
 * @returns {string} fixed html
 */
const fixTableEachBlocks = (html) => {
  if (!html) return html;
  
  let result = html;
  // 1. Remove empty table rows generated by Tiptap
  result = result.replace(/<tr>\s*<td>\s*<p><\/p>\s*<\/td>\s*<\/tr>/g, '');
  
  // 2. Fix the start of the #each block:
  // Find <tr> and everything up to {{#each ...}} WITHOUT crossing another <tr>
  result = result.replace(
    /(<tr>(?:(?!<tr>).)*?)(\{\{#each\s+[^}]+\}\})/g,
    '$2$1'
  );

  // 3. Fix the end of the #each block:
  // Find {{/each}} and everything up to </tr> WITHOUT crossing another </tr>
  result = result.replace(
    /(\{\{\/each\}\})((?:(?!<\/tr>).)*?<\/tr>)/g,
    '$2$1'
  );
...
const processGeneration = async (logId, data) => {
  ...
  try {
    ...
    let fixedHtmlContent = fixTemplateSyntax(template.htmlContent);
    fixedHtmlContent = fixTableEachBlocks(fixedHtmlContent);
    fixedHtmlContent = stripTrailingEmptyParagraphs(fixedHtmlContent);
    ...
    if (data.outputType === 'PDF_SINGLE') {
      ...
      students.forEach((student, index) => {
        const specificData = data.studentCustomData ? data.studentCustomData[student.id] : {};
        const mergedCustomData = { ...data.customData, ...specificData };
        const templateData = { ...teacherData, ...mapStudentVariables(student, mergedCustomData) };
...
const previewEditorHtml = async (htmlContent, cssStyles = '', options = {}) => {
  ...
  if (!templateData) {
    ...
    for (const v of detected) {
      ...
      if (clean === 'nik') { dummyData[clean] = '1234567890123456'; continue; }
      if (clean === 'unit_kerja') { dummyData[clean] = 'SMA Negeri 19 Medan'; continue; }
      if (clean === 'instansi') { dummyData[clean] = 'Dinas Pendidikan Provinsi Sumatera Utara'; continue; }
    ...
    const standardDummy = {
      ...
      nik: '1234567890123456', unit_kerja: 'SMA Negeri 19 Medan',
  ...
  // Compile and render
  let fixedHtmlContent = fixTemplateSyntax(htmlContent);
  fixedHtmlContent = fixTableEachBlocks(fixedHtmlContent);
  fixedHtmlContent = stripTrailingEmptyParagraphs(fixedHtmlContent);
  const compiled = handlebars.compile(fixedHtmlContent);
  let renderedBody;
  try {
